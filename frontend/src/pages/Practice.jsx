...
export default function Practice() {
  ...
  const [targetLetter, setTargetLetter] = useState(
    searchParams.get("letter")?.toUpperCase() || "A",
  );
...
const captureFrame = async () => {
  ...
  try {
    ...
    const res = await fetch("http://127.0.0.1:8000/practice-check", {
      method: "POST",
      body: formData,
    });
    ...
    if (data.correct && data.confidence > 0.65) {
      ...
      setTimeout(() => {
        setTargetLetter(
          alphabet[(alphabet.indexOf(targetLetter) + 1) % 26]
        );
        setIsSuccess(false);
      }, 2000);
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4 space-y-6">
          <div className="glass-card p-8 rounded-[2.5rem] border border-white/5 text-center">
            <p className="text-[10px] font-black text-[var(--primary)] uppercase tracking-[0.3em] mb-4">
              Target Gesture
            </p>
            <motion.div
              key={targetLetter}
              initial={{ scale: 0.5 }}
              animate={{ scale: 1 }}
              className="text-[12rem] font-black text-white italic"
            >
              {targetLetter}
            </motion.div>
            <img
              src={`/signs/${targetLetter}.png`}
              className="w-32 h-32 mx-auto opacity-20 grayscale"
              alt="hint"
            />
          </div>
        </div>
          ...
          <div className="glass-card p-4 rounded-[3rem] bg-black relative overflow-hidden shadow-2xl">
              ...
              <AnimatePresence>
                {isSuccess && (
                  <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="absolute inset-0 bg-[var(--primary)]/20 backdrop-blur-sm flex items-center justify-center z-20"
                  >
                    <div className="bg-white text-black px-8 py-3 rounded-full font-black text-2xl">
                      MATCH CONFIRMED
                    </div>
                  </motion.div>
                )}
            ...
            <div className="grid grid-cols-3 gap-4 mt-6">
              <div className="bg-white/[0.03] p-4 rounded-2xl border border-white/5 text-center">
                <span className="block text-[8px] text-white/20 font-black uppercase">
                  Detected
                </span>
